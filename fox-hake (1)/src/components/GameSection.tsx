...
import { User, RefreshCw, Layers, ArrowRight, ShieldCheck, Cpu } from 'lucide-react';
...
export default function GameSection({ userId, platform, onBack, showLoadingOverlay }: GameSectionProps) {
  ...
  // Generate random predictions conforming to standard row-by-row difficulty rules and upload to RTDB
  const generatePredictions = async (): Promise<void> => {
    ...
    try {
      ...
      console.error("Failed to write to Firebase RTDB:", error);
    }
  };

  const handlePredict = () => {
    showLoadingOverlay('جاري الاتصال بالسيرفر وفك التشفير...', 4000, async () => {
  ...
  const handleReset = () => {
    showLoadingOverlay('جاري إعادة تعيين السيرفر وتفريغ التوقعات...', 3000, async () => {
      try {
        const rRef = ref(rtdb, 'm11');
        await set(rRef, null);
        setHasRevealed(false);
      } catch (error) {
        console.error("Failed to clear Firebase RTDB path:", error);
      }
    });
  };

  // Helper verifying if cell holds safe apple from nested predictions model
  ...
  return (
    ...
    >
      ...
      <div id="gameBoardContainer" className="bg-slate-950 p-4 rounded-2xl border border-slate-800/80 w-full mb-4">
